...
def get_range_torchvision(video_path, start_frame, end_frame):
    ...
    reader.seek(start_sec)

    frames = []
    for frame_data in reader:
        if len(frames) >= (end_frame - start_frame):
            break
        img = frame_data['data'].permute(1, 2, 0).numpy()
        frames.append(img)
...
def get_frames_by_indices(
    ...
    if video_backend == "decord":
        ...
        return get_range_torchvision(video_path, indices[0], indices[-1])
    else:
